...
export default function Home() {
  const { selectedCountry, setSelectedCountry } = usePrismStore();
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {showBoot && (
          ...
          >
            ...
            {/* Loading bar */}
            <div
              className="w-64 h-0.5 rounded overflow-hidden"
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse at center, transparent 40%, rgba(0,2,10,0.6) 100%)',
        }}
      />
      ...
      <div
        className="absolute z-20"
        style={{ bottom: '24px', left: '4%', right: '4%' }}
      >
        <StatusBar />
      </div>

      {/* Right Side Tactical Hub (Card Overlay) */}
      ...
      {/* Corner HUD labels */}
      <motion.div
        className="absolute top-16 left-8 z-10 pointer-events-none"
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.3 }}
        transition={{ delay: 2 }}
      >
        <div className="font-mono-tech" style={{ fontSize: '9px', color: '#00f5ff', letterSpacing: '4px' }}>
          GEOSPATIAL_DATA_LINK // SECURE
        </div>
        <div className="font-mono-tech" style={{ fontSize: '7px', color: '#00f5ff', letterSpacing: '2px', opacity: 0.5 }}>
          LATENCY: 14MS // STATUS: NOMINAL
        </div>
      </motion.div>

      <motion.div
        className="absolute top-16 right-8 z-10 pointer-events-none text-right"
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.3 }}
        transition={{ delay: 2 }}
      >
        <div className="font-mono-tech" style={{ fontSize: '9px', color: '#00f5ff', letterSpacing: '4px' }}>
          ORBITAL_TRAJECTORY_MAPPING
        </div>
        <div className="font-mono-tech" style={{ fontSize: '7px', color: '#00f5ff', letterSpacing: '2px', opacity: 0.5 }}>
      ...
      <motion.div
        className="absolute bottom-32 left-8 z-10 pointer-events-none"
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.2 }}
        transition={{ delay: 2.5 }}
      >
        <div className="font-mono-tech" style={{ fontSize: '9px', color: '#00f5ff', letterSpacing: '4px' }}>
          COMMUNICATION_RELAY_GRID
        </div>
        <div className="font-mono-tech" style={{ fontSize: '7px', color: '#00f5ff', letterSpacing: '2px' }}>
          UPLINK: ACTIVE // STATION: LEO-01
        </div>
      </motion.div>

      <motion.div
        className="absolute bottom-32 right-8 z-10 pointer-events-none text-right"
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.2 }}
        transition={{ delay: 2.5 }}
      >
        <div className="font-mono-tech" style={{ fontSize: '9px', color: '#00f5ff', letterSpacing: '4px' }}>
          THERMAL_SENSING_ARRAYS
        </div>
        <div className="font-mono-tech" style={{ fontSize: '7px', color: '#00f5ff', letterSpacing: '2px' }}>
          SCAN_RATE: 0.8s // CALIBRATED
        </div>
      </motion.div>
