...
def run():
    # Parse current satellite (Note low fuel & capacity)
    current_satellite = Satellite(
        ...
        capabilities=SatelliteCapabilities(
            ...
            bandwidth=0.84,
            reliability=0.93,
            maneuver_capacity_score=0.35,  # Low!
            fuel_or_budget_score=0.30      # Low!
        )
    )

    # Parse target region (High latitude)
    target_region = TargetRegion(
        center_lat=65.0,
        center_lon=21.0,
        bbox=(64.0, 18.0, 66.0, 24.0)
    )

    # Parse simulation state
    sim_state = SimulationState(
        current_time=datetime(2026, 3, 28, 12, 0, 0),
        active_satellites=[current_satellite],
        region_tasks=[],
        degradation_zones=[],
        risk_zones=[]
    )
    ...
    try:
        ...
        constraints = MissionConstraints(
            max_transfer_time_hours=12.0,
            min_elevation_deg=20.0,
            optimization_mode=raw_mode
        )
    except ValueError as e:
        print(f"[!] Validation Error Caught: {e}")
        print("[!] Falling back to 'balanced' mode for the test to continue...\n")
        constraints = MissionConstraints(
            max_transfer_time_hours=12.0,
            min_elevation_deg=20.0,
    ...
    print("Running optimization engine for HIGH_LAT_TARGET & low fuel scenario...")
    ...
    print(f"Fallback optimization mode applied: {result.optimization_mode}")
    print("\n[ BEST PLAN ]")
    ...
    if result.best_feasible_plan and id(result.best_feasible_plan.target_orbit) != id(result.best_plan.target_orbit):
        print("\n================ ALTERNATIVE: BEST FEASIBLE PLAN ================")
        print("The best overall plan is NOT_RECOMMENDED. The following plan is the mathematically best ACCEPTABLE OR HIGH_RISK alternative:\n")
        print(result.best_feasible_plan.explanation)
    elif result.best_feasible_plan is None and result.best_plan.operational_status == "NOT_RECOMMENDED":
        print("\n================ NO FEASIBLE ALTERNATIVE FOUND ================")
        print("All candidate plans violated at least one hard constraint, primarily transfer time and maneuver budget.")
